...
**Known fragility:** the result components map over nested model output largely unguarded (e.g. `path.recommendations.map(...)`), so a response missing one nested key throws and blanks the whole results tree. Tracked as item 14 in `docs/IMPROVEMENT_PLAN.md`.
...
- `src/lib/score.js` — the 80/60/40 score bands and their colours, previously duplicated byte-for-byte across two pages.
- `src/components/results/` — `index.js` holds `ResultCard`, `BulletList`, `ScoreBar`, `ScoreCircle`; `AnalyticsResults`, `JobMatchResults` and `SectionImprovementResults` hold each feature's result tree. The pages are thin controllers now: fetch, errors, quota modal. Put new result markup here, not in a page.
...
`SubscriptionProvider` (in the root layout) checks `/api/subscription/init` on sign-in; a user with no subscription row is treated as needing onboarding and gets the `PricingModal`. `AdProvider` independently fetches `/api/subscription/status` to decide whether to show ads (free tier only, per `AD_CONFIG.showOnlyToFreeUsers`). All subscription API responses are wrapped as `{ status: 'success', data: … }`.
...
Articles are markdown files in `public/assets/*.md`, read at build time via `gray-matter` + `marked` in `src/lib/blog.js`. Titles/dates/read-times come from a **hardcoded `articleMap`** in that file, not from frontmatter — adding an article means adding both the `.md` file and an `articleMap` entry, or `getAllArticles()` silently drops it.
